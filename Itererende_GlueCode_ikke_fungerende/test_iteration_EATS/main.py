...
# r = session.get(input_url, headers=HEADER)
...
tile_url = f"{APP_URL}/v3/{JOB_ID}/tiles/{wsi_id}/level/2/position/45/50"
...
def get_wsi_tile(my_wsi2: dict, x, y):
    """
    ...
    tile_url2 = f"{APP_URL}/v3/{JOB_ID}/tiles/{wsi_id2}/level/{level}/position/{x}/{y}"
...
wsi_tile = get_wsi_tile(my_wsi, 45, 50)
print(wsi_tile)

#wsi_tile_cv = pil_image_to_opencv(wsi_tile)

score, idx = quantification2(image_to_numpy_array(wsi_tile))
